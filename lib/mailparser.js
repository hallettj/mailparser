 ...
 *   <li><b>includeMimeTree</b></li> - if set to true, includes original MIME tree in output
 * </ul>
 *
 * @constructor
...
MailParser.prototype._processMimeTree = function() {
    ...
    if (this.options.includeMimeTree) {
        returnValue.mimeTree = this._acyclicMimeTree(this.mimeTree);
    }
...
/**
 * <p>Produces view of MIME tree suitable for output</p>
 */
MailParser.prototype._acyclicMimeTree = function(node) {
    var returnValue = {
        childNodes: node.childNodes.map(this._acyclicMimeTree.bind(this)),
        headers:    node.parsedHeaders,
    };
    Object.keys(node.meta).forEach(function(k) {
        returnValue[k] = node.meta[k];
    });
    if (typeof node.attachment !== 'undefined') {
        returnValue.attachment = node.attachment;
    }
    if (node.content) {
        returnValue.content = node.content;
    }
    if (node.stream) {
        returnValue.stream = node.stream;
    }
    if (node.streamStarted) {
        returnValue.streamStarted = node.streamStarted;
    }
    return returnValue;
}
